...
var (
	userColumns               = []string{"id", "email", "password", "created_at", "updated_at", "deleted_at"}
...
func (o *User) Media(mods ...qm.QueryMod) mediumQuery {
...
// CreatedByPosts retrieves all the post's Posts with an executor via created_by_id column.
func (o *User) CreatedByPosts(mods ...qm.QueryMod) postQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`posts`.`created_by_id`=?", o.ID),
	)
...
// DeletedByPosts retrieves all the post's Posts with an executor via deleted_by_id column.
func (o *User) DeletedByPosts(mods ...qm.QueryMod) postQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`posts`.`deleted_by_id`=?", o.ID),
	)
...
// UpdatedByPosts retrieves all the post's Posts with an executor via updated_by_id column.
func (o *User) UpdatedByPosts(mods ...qm.QueryMod) postQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`posts`.`updated_by_id`=?", o.ID),
	)
...
func (o *User) TeamMembers(mods ...qm.QueryMod) teamMemberQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`team_members`.`user_id`=?", o.ID),
	)
...
func (userL) LoadMedia(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`media`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatedByPosts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`posts`), qm.WhereIn(`created_by_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadDeletedByPosts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDeletedByPosts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`posts`), qm.WhereIn(`deleted_by_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.DeletedByPosts = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &postR{}
			}
			foreign.R.DeletedBy = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.DeletedByID) {
				local.R.DeletedByPosts = append(local.R.DeletedByPosts, foreign)
				if foreign.R == nil {
					foreign.R = &postR{}
				}
				foreign.R.DeletedBy = local
				break
			}
		}
	}
...
// LoadUpdatedByPosts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUpdatedByPosts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`posts`), qm.WhereIn(`updated_by_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UpdatedByPosts = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &postR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UpdatedByID {
				local.R.UpdatedByPosts = append(local.R.UpdatedByPosts, foreign)
				if foreign.R == nil {
					foreign.R = &postR{}
				}
...
func (userL) LoadTeamMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`team_members`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddMedia(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Medium) error {
	var err error
...
func (o *User) AddCreatedByPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	var err error
	for _, rel := range related {
		if insert {
...
// AddDeletedByPosts adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DeletedByPosts.
// Sets related.R.DeletedBy appropriately.
func (o *User) AddDeletedByPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.DeletedByID, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &userR{
			DeletedByPosts: related,
		}
	} else {
		o.R.DeletedByPosts = append(o.R.DeletedByPosts, related...)
	}
...
// SetDeletedByPosts removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.DeletedBy's DeletedByPosts accordingly.
// Replaces o.R.DeletedByPosts with related.
// Sets related.R.DeletedBy's DeletedByPosts accordingly.
func (o *User) SetDeletedByPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	query := "update `posts` set `deleted_by_id` = null where `deleted_by_id` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.DeletedByPosts {
			queries.SetScanner(&rel.DeletedByID, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.DeletedByPosts = nil
	}
	return o.AddDeletedByPosts(ctx, exec, insert, related...)
}

// RemoveDeletedByPosts relationships from objects passed in.
// Removes related items from R.DeletedByPosts (uses pointer comparison, removal does not keep order)
// Sets related.R.DeletedBy.
func (o *User) RemoveDeletedByPosts(ctx context.Context, exec boil.ContextExecutor, related ...*Post) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.DeletedByID, nil)
		if rel.R != nil {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("deleted_by_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.DeletedByPosts {
			if rel != ri {
				continue
			}

			ln := len(o.R.DeletedByPosts)
			if ln > 1 && i < ln-1 {
				o.R.DeletedByPosts[i] = o.R.DeletedByPosts[ln-1]
			}
			o.R.DeletedByPosts = o.R.DeletedByPosts[:ln-1]
			break
		}
	}
...
// AddUpdatedByPosts adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UpdatedByPosts.
// Sets related.R.UpdatedBy appropriately.
func (o *User) AddUpdatedByPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		o.R = &userR{
			UpdatedByPosts: related,
		}
	} else {
		o.R.UpdatedByPosts = append(o.R.UpdatedByPosts, related...)
	}
